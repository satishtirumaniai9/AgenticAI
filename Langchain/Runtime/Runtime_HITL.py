...
print("Brain is connected and ready to use.")
...
result = agent_with_context.invoke(
    ...
    context=CineBotContext(user_name="Satish"),   # injected at invocation time
)
...
result = logged_agent.invoke(
    ...
    context=CustomerContext(user_id="satish"),
)
...
def get_interrupts(result):
    return result.get("__interrupt__", ()) if isinstance(result, dict) else result.interrupts
...
unguarded_agent = create_agent(model=model, tools=[cancel_booking])
...
guarded_agent.invoke({"messages": [("user", "Cancel booking BK1042")]}, config=config2, version="v2")
...
print("Small refund, paused?", bool(get_interrupts(r_small)))
...
print("Large refund, paused?", bool(get_interrupts(r_large)))
